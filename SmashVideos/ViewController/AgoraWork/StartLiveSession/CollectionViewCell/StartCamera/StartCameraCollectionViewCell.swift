//
//  StartCameraCollectionViewCell.swift
//  SmashVideos
//
...
import UIKit
import SDWebImage
class StartCameraCollectionViewCell: UICollectionViewCell {
    ...
    @IBOutlet var btnVisitor: UIButton!
    
    @IBOutlet weak var comentBtn: UIButton!
    @IBOutlet weak var pause_liveView: UIView!
    
    @IBOutlet var btnMenu: UIButton!
    
    @IBOutlet var moreView: UIView!
    
    @IBOutlet var visitorStackView: UIStackView!
    ...
    override func awakeFromNib() {
        ...
        tblComment.register(UINib(nibName: "LiveCommentTableViewCell", bundle: nil), forCellReuseIdentifier: "LiveCommentTableViewCell")
        tblComment.register(UINib(nibName: "giftNewTableViewCell", bundle: nil), forCellReuseIdentifier: "giftNewTableViewCell")
...
extension StartCameraCollectionViewCell:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "LiveCommentTableViewCell", for: indexPath) as! LiveCommentTableViewCell
            cell.userName.text = "Welcome to MUSICTOK LIVE! Have fun interacting with your viewers in real time. Hosts must be 18 or older to go LIVE. Viewers must be 18 or older to recharge and send Gifts. Remember to create a safe viewing experience by following our Community Guidelines."
            cell.userName.numberOfLines = 0
            cell.comment.isHidden = true
            cell.userName.textDropShadow()
            cell.userName.font = .boldSystemFont(ofSize: 12)
//                  cell.userImg.sd_imageIndicator = SDWebImageActivityIndicator.white
            cell.userImg.image = UIImage(named: "icon-6")//
            return cell
        ...
        if type == "comment" {
            ...
            return cell
              ...
              cell.comment.isHidden = true
//                  cell.userImg.sd_imageIndicator = SDWebImageActivityIndicator.white
              cell.userImg.image = UIImage(named: "joined")// .sd_setImage(with: URL(string: (AppUtility?.detectURL(ipString: (self.commentsArr[indexPath.row]["userPicture"] as? String ?? "unknown User")))!), placeholderImage: #imageLiteral(resourceName: "noUserImg"))
...
////        let image = UIImage(named: "icon-6")
...
//        label.font = .boldSystemFont(ofSize: 12)// .systemFont(ofSize: 14.0)
...
//        label.leadingAnchor.constraint(equalTo: headerView.leadingAnchor, constant: 8).isActive = true
//        label.trailingAnchor.constraint(equalTo: headerView.trailingAnchor, constant: -8).isActive = true
//        label.bottomAnchor.constraint(equalTo: headerView.bottomAnchor, constant: 0).isActive = true
//        // label.heightAnchor.constraint(equalToConstant: 18).isActive = true
//
//
//        label.text = "Welcome to MUSICTOK LIVE! Have fun interacting with your viewers in real time. Hosts must be 18 or older to go LIVE. Viewers must be 18 or older to recharge and send Gifts. Remember to create a safe viewing experience by following our Community Guidelines."
//
//
//
//        return headerView
