//
//  StartLiveRoomViewController.swift
//  SmashVideos
//
...
class StartLiveRoomViewController: UIViewController, UIGestureRecognizerDelegate {
...
//            placeholderView.isHidden = (videoSessions.count == 0 ? false : true)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        self.pause_liveView.isHidden = true
...
//            NotificationCenter.default.post(name: Notification.Name("removeLiveUserNoti"), object:nil,userInfo:nil)
    ...
    func Comments(){
        let cell = self.liveCollectionView.cellForItem(at: IndexPath(row: 0, section: 0))as? StartCameraCollectionViewCell
...
//                    cell?.tblComment.reloadData()
    ...
    @objc func menuButtonPressed(sender:UIButton){
        ...
        myViewController.video = self.video
        ...
        myViewController.callback = { (text,id) -> Void in
            ...
            if text == "mic" {
                ...
                if self.video == "1" {
                    ...
                    self.pauseView(pause: false)
//                    self.pause_liveView.isHidden = true
                }else {
                    self.agoraKit.disableVideo()
                    self.pauseView(pause: true)
//                    self.pause_liveView.isHidden = false
                }
            }else {
    ...
    @objc func visitorButtonPressed(sender:UIButton){
        let myViewController = VisitorLiveViewController(nibName: "VisitorLiveViewController", bundle: nil)
...
//
    ...
    func pauseView(pause : Bool) {
        let visibleCells = liveCollectionView.visibleCells.compactMap { $0 as? StartCameraCollectionViewCell }
        
        guard visibleCells.count > 0 else { return }
        
        let visibleFrame = CGRect(x: 0, y: liveCollectionView.contentOffset.y, width: liveCollectionView.bounds.width, height: liveCollectionView.bounds.height)
        
        let visibleCell = visibleCells
            .filter { visibleFrame.intersection($0.frame).height >= $0.frame.height / 2 }
            .first
        if pause == true {
            visibleCell?.pause_liveView.isHidden = false
            self.liveCollectionView.isScrollEnabled = false
        }else {
            visibleCell?.pause_liveView.isHidden = true
            self.liveCollectionView.isScrollEnabled = true
        }
...
private extension StartLiveRoomViewController {
...
extension StartLiveRoomViewController: AgoraRtcEngineDelegate {
...
extension StartLiveRoomViewController:UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
...
//        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == 1 {
            ...
            cell.btnVisitor.addTarget(self, action: #selector(visitorButtonPressed(sender:)), for: .touchUpInside)
...
//                cell.lblUserLive.text = "0"
                ...
                LiveUsers.observe(DataEventType.value, with: { (snapshot) in
                        ...
                        if likes == 0 {
                            ...
                            self.generateAnimatedViews(index: indexPath.row)
...
//            cell.userBackView.addGestureRecognizer(tap1)
    ...
    func generateAnimatedViews(index: Int){
        let cell = liveCollectionView.cellForItem(at: IndexPath(row: index, section: 0))as? StartCameraCollectionViewCell
