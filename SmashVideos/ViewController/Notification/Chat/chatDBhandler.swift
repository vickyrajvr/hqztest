...
//
//  Created by Zubair Ahmed on 22/08/2022.
//  Copyright © 2022 Zubair  Ahmed. All rights reserved.
//
...
//    }
    ...
    func sendMessage(senderID: String, receiverID: String,senderName: String, message: String, status: String, time: Any,textTimestamp: Any, type: String, completionHandler: @escaping (_ result: Bool) -> Void)
    {
        print("chat send msgs values")
        
        let messages  = Database.database().reference().child("chat")
        let chatOf = messages.child(senderID+"-"+receiverID).childByAutoId()
        var value = [String : Any]()
        if type == "image" {
            value = ["chat_id": "\(chatOf.key!)","pic_url":message,"receiver_id":receiverID,"sender_id":senderID,"sender_name":senderName ,"status": status, "text": "", "time": time,"timestamp": "\(textTimestamp)", "type": "\(type)"] as [String : Any]
        }else {
            value = ["chat_id": "\(chatOf.key!)","pic_url":message,"receiver_id":receiverID,"sender_id":senderID,"sender_name":senderName ,"status": status, "text": message, "time": time,"timestamp": "\(textTimestamp)", "type": "\(type)"] as [String : Any]
        }
//        let value = ["chat_id": "\(chatOf.key!)","pic_url":message,"receiver_id":receiverID,"sender_id":senderID,"sender_name":senderName ,"status": status, "text": message, "time": time,"timestamp": "\(textTimestamp)", "type": "\(type)"] as [String : Any]
        
        print("chat send msgs values :\(value)")
        chatOf.updateChildValues(value) { (error, Ref) in
            if error != nil
            {
                print("error in send message fromId: ", error!.localizedDescription)
    ...
    func deleteMessage(senderID: String, receiverID: String,senderName: String, message: String, status: String, time: Any,textTimestamp: Any, type: String,chat_id: String, completionHandler: @escaping (_ result: Bool) -> Void)
    {
        print("chat send msgs values")
        
        let messages  = Database.database().reference().child("chat")
        let chatOf = messages.child(senderID+"-"+receiverID).child(chat_id)
        let value = ["chat_id": "\(chat_id)","pic_url":"","receiver_id":receiverID,"sender_id":senderID,"sender_name":senderName ,"status": status, "text": message, "time": time,"timestamp": "\(textTimestamp)", "type": "\(type)"] as [String : Any]
        ...
        chatOf.updateChildValues(value) { (error, Ref) in
            ...
            let toId = messages.child(receiverID+"-"+senderID).child("\(chat_id)")
            let value = ["chat_id": "\(chat_id)","pic_url":message,"receiver_id":receiverID,"sender_id":senderID,"sender_name":senderName ,"status": status, "text": message, "time": time,"timestamp": "\(textTimestamp)", "type": "\(type)"] as [String : Any]
            toId.updateChildValues(value) { (error, Ref) in
                if error != nil
                {
                    print("error in send message toId: ", error!.localizedDescription)
   ...
   */
    //MARK:- send user-Chat
    func userChatInbox(senderID: String, receiverID: String, image: String, name: String, message: String, timestamp: Any, date:String,status:String,completionHandler: @escaping (_ result: Bool) -> Void)
     {
         let Inbox  = Database.database().reference().child("Inbox")
         
         let fromId = Inbox.child(senderID).child(receiverID)
         let value = ["rid": "\(receiverID)", "pic": "\(image)", "name": "\(name)", "msg": "\(message)", "timestamp": timestamp,"date": date,"status": status] as [String : Any]
         print("chat send msgs values :\(value)")
         fromId.updateChildValues(value) { (error, Ref) in
             if error != nil
             {
            ...
            let senderProfile = (AppUtility?.detectURL(ipString: myUser![0].profile_pic))!
             ...
             let value = ["rid": "\(senderID)", "pic": "\(senderProfile)", "name": "\(senderName!)", "msg": "\(message)", "timestamp": timestamp,"date": date,"status": status] as [String : Any]
    ...
    */
    ...
    func sendImage(senderName : String , senderID: String, receiverID: String, image: Data, seen: Bool, time: Any, type: String, completionHandler: @escaping (_ result: Bool,_ url: String) -> Void)
    {
//        AppUtility.shared.showLoader(message: "Please wait...")
...
//                    AppUtility.shared.hideLoader()
                ...
                else
                {
                    ///*
                    ///
                    self.sendMessage(senderID: senderID, receiverID: receiverID, senderName: senderName, message: "\(url!)", status: "0", time: "", textTimestamp: "\(time)", type: type){ (isSuccess) in
                        completionHandler(true, "\(url!)")
//                        AppUtility.shared.hideLoader()
                    }
                    
                    
//                    self.sendMessage(uid: senderID, merchantId: merchantId, message: "\(url!)", seen: seen, time: time, type: type) { (isSuccess) in
//                        completionHandler(true, "\(url!)")
//                        AppUtility.shared.hideLoader()
//                    }
                   // */
