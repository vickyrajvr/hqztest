...
class soundsViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    @IBAction func btnDiscoverAction(_ sender: Any) {
        ...
        btnDiscover.setTitleColor(UIColor(named: "theme"), for: .normal)
        btnDiscoverBottomView.backgroundColor = UIColor(named: "theme")
    ...
    @IBAction func btnFavAction(_ sender: Any) {
        ...
        btnFavBottomView.backgroundColor = UIColor(named: "theme")
        btnFav.setTitleColor(UIColor(named: "theme"), for: .normal)
    ...
    @objc func btnSelectAction(_ sender : UIButton) {
        ...
        UserDefaultsManager.shared.sound_id = newObj.id
...
//    MARK:- BTN ALL ACTION
    ...
    func getFavSounds(){
        ...
        ApiHandler.sharedInstance.showFavouriteSounds(user_id: UserDefaultsManager.shared.user_id) { (isSuccess, response) in
            self.loader.isHidden = true
            if isSuccess{
                if (response?.value(forKey: "code") as! NSNumber) == 200{
                    ...
                    for dic in msgDic{
                        ...
                        let total_videos = soundObj.value(forKey: "total_videos")  as? String
                        let obj = soundsMVC(id: id, audioURL: audioUrl, duration: duration, name: name, description: description, thum: thum, section: section, uploaded_by: uploaded_by, created: created, favourite: "", publish: publish, total_videos: total_videos ?? "")
...
//                    print("!200: ",response?.value(forKey: "msg")!)
    ...
    func GetAllSounds(){
        ...
        self.loader.isHidden = false
        ApiHandler.sharedInstance.showSounds(user_id: UserDefaultsManager.shared.user_id, starting_point: startingPoint) { (isSuccess, response) in
            self.loader.isHidden = true
            if isSuccess{
                if response?.value(forKey: "code") as! NSNumber == 200{
                    ...
                    for i in 0 ..< soundMsg.count{
                        ...
                        for j in 0 ..< soundsObj.count{
                            ...
                            let total_videos = soundsData.value(forKey: "total_videos")  as? String
                            let soundObj = soundsMVC(id: id, audioURL: audioUrl, duration: duration, name: name, description: description, thum: thum, section: section, uploaded_by: uploaded_by, created: created, favourite: "\(favourite!)", publish: publish, total_videos: total_videos ?? "")
