...
class CoinShareViewController: UIViewController,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout {
   ...
   // var coin_receiverID = ""
    ...
    override func viewDidLoad() {
        ...
        self.lblShare.text =  "Send gift to \(StaticData.obj.liveUserName.isEmpty ? userName : StaticData.obj.liveUserName)"
        
        print(" UserDefaultsManager.shared.wallet", UserDefaultsManager.shared.wallet)
        
        
        self.TotalCoin.text! = UserDefaultsManager.shared.wallet
    ...
    @IBAction func btnRecchargeAction(_ sender: Any) {
        ...
        if  Int(self.gift_price) ?? 0  > Int(UserDefaultsManager.shared.wallet) ?? 0{

            print("Please Recharge")
            self.alertModule(title: "Recharge", msg: "Please recharge, you have not enough coins")
            return
        }
        self.send_Coin(sticker_id:self.sticker_id , gift_count: self.gift_price)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let CoverImgURL =  obj["icon"] as! String
        print("CoverImgURL",CoverImgURL)
        
        cell.imgCoin.layer.cornerRadius = cell.imgCoin.frame.height / 2
        cell.imgCoin.clipsToBounds = true
        cell.imgCoin.contentMode = .scaleAspectFill
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.gift_price = obj["coin"] as! String
        print("gift_count",gift_price)
...
//        return CGSize(width:collectionViewAllCoinStickers.frame.size.width / 4, height:100)
    ...
    func send_Coin(sticker_id:String,gift_count:String){
        ...
        ApiHandler.sharedInstance.sendGifts(sender_id:userID, receiver_id: StaticData.obj.liveUserID, gift_id: sticker_id, gift_count:"1"){ (isSuccess, response) in
            if isSuccess{
                self.loader.isHidden = true
